/**
  ...
  Modification par Ludovic Olanier

  ligne 150	SUPA � X-500 Y-1 Z-1
  ligne 492     var TC = "DMG";
  ligne 494     var ST = 220000;
  ligne 1230	Ajout de OFFN=- avant G41
...
//SUPA � X-500 Y-1 Z-1
machineConfiguration.setHomePositionX(-500);
machineConfiguration.setHomePositionY(-1);
machineConfiguration.setRetractPlane(-1); 
  ...
  if (true) { // stock - workpiece
    ...
    if (delta.isNonZero()) {
      writeBlock(
        "WORKPIECE" + "(" + "," + "\"" + "C" + "\"" + ",," + "\"" + "BOX" + "\""  + "," + "112" + "," + xyzFormat.format(workpiece.upper.z) + "," + xyzFormat.format(workpiece.lower.z) + "," + "80" +
...
function onLinear(_x, _y, _z, feed) {
  ...
  if (x || y || z) {
	...
	//OFFN=- permet de ne pas avoir les jauges d'outils �gale � 0 dans la machine
	writeBlock("OFFN=-" + xyzFormat.format(tool.diameter/2)); //D�cale le parcours du rayon de la fraise
...
function onClose() {
  ...
  writeBlock(gMotionModal.format(0), "SUPA", "Z" + machineConfiguration.getRetractPlane(), "D0"); // retract
//  writeBlock(gMotionModal.format(0), "SUPA", "X-500", "Y-1"); //G0 SUPA X-500 Y-1
