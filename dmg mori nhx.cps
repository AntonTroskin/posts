/**
  ...
  DMG Mori MORISEIKI TONI post processor configuration.

  $Revision: 42622 c34293f826e4bfeffedc4842dce8aafb1fb3293e $
  $Date: 2020-01-09 16:05:53 $
  
  FORKID {13F512FD-438D-404B-9F4F-9414695F66F8}
*/

description = "DMG Mori NHX with CELOS control";
vendor = "DMG MORI";
...
longDescription = "DMG Mori NHX series horizontal machining center with a B-axis and a CELOS control with MAPPS.";
...
properties = {
  ...
  useSmoothingType: "I", // 1 = G05.1 Q1 output, 2 = G05 P10000 output.
  ...
  useG54x4: false, // supports G54.4 for Workpiece Error Compensation
  makeBAxisOtherWay: false, // make the B-axis rotate the opposite way
  gotContinuousBAxis: true, // B-axis is continuous with no limits
  ...
  useG30: true, // uses G30 to position home instead of G28
  palletChangeCode: "none", // pallet code to output at end of program
...
propertyDefinitions = {
  ...
  sequenceNumberOnlyOnToolChange: {title:"Sequence number at tool change only", description:"Use sequence numbers at a tool change only.", type:"boolean"},
  ...
  useSmoothing: {title:"High accuracy control", description:"Enables high-speed, high-accuracy control.", type:"boolean"},
  useSmoothingType: {
    title: "High accuracy control type",
    description: "High accuracy control type I (G5.1 Q1) or type II (G5 P10000).",
    type: "enum",
    values:[
      {title:"Type I", id:"I"},
      {title:"Type II", id:"II"}
  ...
  useG54x4: {title:"Use G54.4", description:"Supports G54.4 for workpiece error compensation.", type:"boolean"},
  makeBAxisOtherWay: {title:"Reverse direction of B-axis", description:"Positive value moves B-axis in CCLW direction.", type:"boolean"},
  gotContinuousBAxis: {title:"B-axis is continuous", description:"B-axis is contouring axis with no physical limits.  'No' defines an indexing table with 1 degree increments.", type:"boolean"},
  ...
  palletChangeCode: {
    title: "Pallet change code",
    description: "Pallet change code to output at end of program. ",
    type: "enum",
    values:[
      {title:"No pallet changer", id:"none"},
      {title:"M60 2 pallet system", id:"60"},
      {title:"M160 5 pallet system", id:"160"}
    ]
  },
  useRigidTapping: {
...
var numberOfToolSlots = 240; // number of available tools
...
function onOpen() {
  ...
  gSmoothing = properties.useSmoothingType == "II" ? 2 : 1;
  rSmoothing = 0;

  if (true) {
    var bAxis;
    if (properties.gotContinuousBAxis) {
      bAxis = createAxis({coordinate:1, table:true, axis:[0, (properties.makeBAxisOtherWay ? -1 : 1) * -1, 0], cyclic:true, preference:1});
    } else {
      bAxis = createAxis({coordinate:1, table:true, axis:[0, (properties.makeBAxisOtherWay ? -1 : 1) * -1, 0], cyclic:true, range:[0, 360]});
...
function setSmoothing(mode) {
  ...
  var gType = mode ? gSmoothing : activeSmoothing;
  currentSmoothing = mode;
  activeSmoothing = gType;
  writeG332(mode);
  if (gType == 1) {
    writeBlock(gFormat.format(5.1), mode ? "Q1" : "Q0");
  } else {
    writeBlock(gFormat.format(5), mode ? "P10000" : "P0");
  }
  return true;
}

function writeG332(mode) {
  if (!mode) {
    writeBlock(gFormat.format(332), "R3."); // reset G332 setting to machine default
  } else if (hasParameter("operation-strategy") && (getParameter("operation-strategy") == "drill")) {
    writeBlock(gFormat.format(332), "R3."); // reset G332 setting to machine default
  } else if (rSmoothing != 0) {
    writeBlock(gFormat.format(332), "R" + rSmoothing + ".");
    ...
    if (tolerance > 0) {
      ...
      if ((stockToLeave > stockToLeaveThreshold) && (verticalStockToLeave > stockToLeaveThreshold)) {
        workMode = 1; // roughing, 2 = intermediate not currently used
      } else {
      ...
      writeBlock(gFormat.format(332), "R" + workMode + "."); // set tolerance mode
    } else {
      writeBlock(gFormat.format(332), "R3."); // reset G332 setting to machine default
    }
  } else {
...
function getWorkPlaneMachineABC(workPlane, _setWorkPlane, rotate) {
  ...
  var abcTest = new Vector(abc.x, abc.y, abc.z); // affects output if not copied to new vector
  if (!properties.gotContinuousBAxis) {
...
function onParameter(name, value) {
  if (name == "probe-output-work-offset") {
    ...
    if (sText2[0] == "SMOOTH") {
      var num = parseNumbers(sText2[1], 2);
      if ((num.length < 1) || (num.length > 2)) {
        error(localize("Invalid SMOOTH command" + ": " + value));
        return;
      }
      if ((num[0] < 0) || (num[0] > 2)) {
        error(localize("SMOOTH type must be 0, 1 or 2" + ": " + value));
        return;
      }
      if (num.length == 2) {
        if ((num[1] < 0) || (num[1] > 4)) {
          error(localize("SMOOTH accuracy must be 1-4" + ": " + value));
          return;
        }
        rSmoothing = num[1];
      }
      gSmoothing = num[0];
      smoothing = gSmoothing != 0;
    } else {
...
function onSection() {
  ...
  // set coolant after tool change so it gets a chance to start prior to first cut
...
function onLinear5D(_x, _y, _z, _a, _b, _c, feed) {
  if (!currentSection.isOptimizedForMachine() || !properties.gotContinuousBAxis) {
...
function onCommand(command) {
  ...
  case COMMAND_LOCK_MULTI_AXIS:
    writeBlock(gFormat.format(4),"x2.");// dwell 2s
  ...
  case COMMAND_STOP_CHIP_TRANSPORT:
    writeBlock(mFormat.format(201));
    return;
  case COMMAND_BREAK_CONTROL:
    writeBlock(gFormat.format(103));
    return;
...
function writeRetract() {
  ...
  var _useG28 = true; // can be either true or false
  ...
  if (words.length > 0) {
    if (_useG28) {
      gAbsIncModal.reset();
      writeBlock(gFormat.format(homeGCode), gAbsIncModal.format(91), words);
      writeBlock(gAbsIncModal.format(90));
    } else {
      gMotionModal.reset();
...
function onClose() {
  ...
  if (properties.palletChangeCode != "none") {
    writeBlock(mFormat.format(parseFloat(properties.palletChangeCode))); // call up RPP pallet system
  }
