...
function onOpen() {
  ...
  if (true) {
    ...
    var bAxis = createAxis({coordinate:1, table:false, axis:[0, -1, 0], range:[-35.0001, 110.0001], preference:1});
  ...
  machineConfiguration.setRetractPlane(-.1); // home position Z
  machineConfiguration.setHomePositionX(0); // home position X
  machineConfiguration.setHomePositionY(-.1); // home position Y
...
function defineWorkPlane(_section, _setWorkPlane) {
  ...
  if (machineConfiguration.isMultiAxisConfiguration()) { // use 5-axis indexing for multi-axis mode
    ...
    if (_section.isMultiAxis()) {
      ...
      abc = _section.workPlane.getTransposed().eulerZYX_R;
...
function writeRetract() {
  ...
  if (block) {
    gMotionModal.reset();
    writeBlock(gMotionModal.format(0), "SUPA " + block + "D0"); // retract
  }
  zOutput.reset();
}
