...
propertyDefinitions = {
  ...
  showSequenceNumbers: {title:"Use sequence numbers", description:"Use sequence numbers for each block of outputted code.", group:9, type:"boolean"},
  sequenceNumberStart: {title:"Start sequence number", description:"The number at which to start the sequence numbers.", group:9, type:"integer"},
  sequenceNumberIncrement: {title:"Sequence number increment", description:"The amount by which the sequence number is incremented by in each block.", group:9, type:"integer"},
  optionalStop: {title:"Optional stop", description:"Outputs optional stop code during when necessary in the code.", group:1, type:"boolean"},
  ...
  useParametricFeed:  {title:"Parametric feed", description:"Specifies the feed value that should be output using a Q value.", group:9, type:"boolean"},
  showNotes: {title:"Show notes", description:"Writes operation notes as comments in the outputted code.", group:9, type:"boolean"},
  ...
  useAAxis: {title: "Use A-axis", description: "Specifies whether to use the A axis.", group:1, type: "boolean"},
  useTrunnion: {title: "Use AC-trunnion", description: "Enables a trunnion table with an A and C-axis.", group:1, type: "boolean"},
  probingType: {title: "Probing type", description: "Specified what probing cycles are used on the machine.", group:9, type: "enum", values: [{title: "Renishaw", id: "Renishaw"}, {title: "Blum", id: "Blum"}]},
  washdownCoolant: {title: "Washdown coolant", description: "Specifies whether washdown coolant should be used and where it is output.", group:3, type: "enum", values:[{title: "Off", id: "off"}, {title: "Always on", id: "always"}, {title: "End of operation", id: "operationEnd"}, {title: "Program end", id:"programEnd"}]},
  usePitchForTapping: {title:"Use Pitch/TPI for tapping", description:"Specifies if high accuracy mode should be used.", group:1, type:"boolean"},
  /*outputUnit: {
    title: "Output units",
...
var cyclefeedOutput = createVariable({prefix:"F", force:false}, feedFormat);
...
function onOpen() {
  ...
  if (true) {
    ...
    writeComment("BROTHER  - " + d.toLocaleDateString() + "  - " +
...
function onSection() {
  ...
  if (true) { //anton tool.description
    // writeWords(("(" + tool.description + ")").toUpperCase());
    var showToolZMin = true; //anton
    if (showToolZMin) {
      if (is3D()) {
        var numberOfSections = getNumberOfSections();
        ...
        writeWords(("(" + tool.description + "    D=" + xyzFormat.format(tool.diameter) + "   ZMIN=" + zRange.getMinimum() + ")").toUpperCase());
  ...
  if (insertToolCall) {
    ...
    writeBlock(mFormat.format(6),//(gFormat.format(100),
      "T" + toolFormat.format(tool.number)
      // mFormat.format(6), //anton
    );
    ...
    if (properties.preloadTool) {
      ...
      var irankis = ""
      if (nextTool) {
        irankis = "T" + toolFormat.format(nextTool.number);
      } else {
        // preload first tool
        var section = getSection(0);
        var firstToolNumber = section.getTool().number;
        if (tool.number != firstToolNumber) {
          irankis = "T" + toolFormat.format(firstToolNumber);
        }
      }
    }
  ...
  setCoolant(tool.coolant); //anton ----hereeee
  ...
  if (insertToolCall || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    writeBlock(gPlaneModal.format(17)); // anton papildomas pozicionavimas pries cikla
...
function onCyclePoint(x, y, z) {
  ...
  if (isProbeOperation()) {
    ...
    if (workOffset > 99) {
      ...
      probeWorkOffsetCode = ((properties.probingType == "Renishaw") ? (workOffset + ".") : (workOffset + 53)); // G54->G59
    }
  }
  ...
  if (isFirstCyclePoint() || isProbeOperation()) {
    ...
    case "counter-boring":
      ...
      break;
    case "chip-breaking":
      if (false) { //((cycle.accumulatedDepth < cycle.depth) || (P > 0)) {
    ...
    case "right-tapping-with-chip-breaking": //anton issiaiskinti ar veikia tap with chip break
      if (false) { //cycle.accumulatedDepth < cycle.depth
    ...
    case "probing-xy-outer-corner":
      ...
      if (properties.probingType == "Renishaw") {
        ...
        writeBlock(
          ...
          yOutput.format(cornerY), getProbingArguments(cycle, probeWorkOffsetCode)
        );
      }
      break;
...
function getProbingArguments(cycle, probeWorkOffsetCode) {
  ...
  return [
    ...
    conditional(probeWorkOffsetCode && probeWCS, (properties.probingType == "Renishaw" ? "S" : "W") + probeWorkOffsetCode)
  ];
}
