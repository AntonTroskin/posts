...
longDescription = "Anton post for Himold 750.";
...
properties = {
  ...
  has5Axis: true, // specifies if multi-axis toolpath should be output	Anton
...
propertyDefinitions = {
  ...
  has5Axis: {title:"Has 5-axis", description:"Specifies if multi-axis toolpaths are supported.", type:"boolean"},	//Anton			
...
var singleLineCoolant = true; // Anton -f specifies to output multiple coolant codes in one line rather than in separate lines
// samples:
...
var forceMultiAxisIndexing = true; // Anton -f force multi-axis indexing for 3D programs
var maximumLineLength = 80; // the maximum number of charaters allowed in a line
var minimumCyclePoints = 5; // minimum number of points in cycle operation to consider for subprogram
var cancelTiltFirst = true; // Anton -f cancel G68.2 with G69 prior to G54-G59 WCS block
...
function onOpen() {
  ...
  if (properties.has5Axis) { // note: setup your machine here Anton
  ...
  if (programName) {
    ...
    if (programComment) {
      writeln("(" + "O" + oFormat.format(programId) + " (" + filterText(String(programComment).toUpperCase(), permittedCommentChars) + ")" + ")"); //Anton
    } else {
