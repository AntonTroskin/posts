...
properties = {
  ...
  cycle800Mode: "57", // specifies the mode to use for CYCLE800
...
function onOpen() {
  ...
  if (true) {
    ...
    var bAxis = createAxis({coordinate:1, table:false, axis:[0, 1, 0], range:[-90.0001, 90.0001], preference:1});
...
*/
    ...
    var _MVAR = (!singleLine ? "_MVAR=" : ""); // CYCLE TYPE
    var _MA = (!singleLine ? "_MA=" : ""); // number of the axis, X=1, Y=2, Z=3

    if (tool.number >= 100) {
      error(localize("Tool number is out of range for probing. Tool number must be below 100."));
      return;
    }
    var _PRNUM = (!singleLine ? "_PRNUM=" : "") + toolProbeFormat.format(tool.number); // Probingtyp, Probingnumber. 3 digits. 1st = (0=Multiprobe, 1=Monoprobe), 2nd/3rd = 2digit Probing-Tool-Number
    var _KNUM = (!singleLine ? "_KNUM=" + xyzFormat.format(9999) : xyzFormat.format(10001)); // automatically input in active workOffset. e.g. _KNUM=1 (G54)
    var _VMS = (!singleLine ? "_VMS=" : "") + xyzFormat.format(0); // Feed of probing. 0=150mm/min, >1=300m/min
    var _TSA = (!singleLine ? "_TSA=" : "") + xyzFormat.format(1); // tolerance (trust area)
    var _NMSP = (!singleLine ? "_NMSP=" : "") + xyzFormat.format(1); // number of measurements at same spot
    var _ID = (!singleLine ? "_ID=" : "") + xyzFormat.format(-Math.abs(cycle.clearance - (z - cycle.depth))); // incremental depth infeed in Z, direction over sign (only by circular boss, wall resp. rectangle and by hole/channel/circular boss/wall with guard zone)
    ...
    var _SETV0 = (!singleLine ? "_SETV[0]=" : "") + (cycle.width1 != undefined ? xyzFormat.format(cycle.width1) : (singleLine ? xyzFormat.format(0) : "")); // nominal value in X
    var _SETV1 = (!singleLine ? "_SETV[1]=" : "") + (cycle.width2 != undefined ? xyzFormat.format(cycle.width2) : ""); // nominal value in Y
    var _DMODE = 0;
    // _FA is always in mm
    ...
    case "probing-x":
    case "probing-y":
      var _SETVAL = (!singleLine ? "_SETVAL=" : "") + xyzFormat.format((cycleType == "probing-x" ? x : y) + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2));
      ...
      if (!singleLine) {
        ...
        writeBlock(
          "CYCLE978(" + ([
            _MVAR, _KNUM, "", _PRNUM, _SETVAL, _FA, _TSA, _MA, "", _NMSP,
            (properties.toolAsName ? "\"" + tool.description + "\"" : "\"\""),
            "", "", "", "", "", "", "", "", "", 0, ""].join(cycleSeparator)) + ")"
        );
      }
      break;
    case "probing-z":
      var _SETVAL = (!singleLine ? "_SETVAL=" : "") + xyzFormat.format(z - cycle.depth);
      _MVAR += xyzFormat.format(100);
      ...
      if (!singleLine) {
        ...
        writeBlock("CYCLE978(" + ([
          _MVAR, _KNUM, _PRNUM, _SETVAL, _FA, _TSA, _MA, undefined, _NMSP,
          (properties.toolAsName ? tool.description : ""),
          "", "", "", "", "", "", "", "", "", "", 0, ""].join(cycleSeparator)) + ")"
        );
      }
    ...
    case "probing-x-channel-with-island":
      var _SETVAL = (!singleLine ? "_SETVAL=" : "") + (cycle.width1 != undefined ? xyzFormat.format(cycle.width1) : ""); // nominal value (only by hole/circular boss/channel/wall)
      _MVAR += xyzFormat.format(cycleType == "probing-x-channel" ? 103 : 1003);
      _MA += xyzFormat.format(1);
      writeBlock(conditional(cycleType == "probing-x-channel", gMotionModal.format(1) + " " + zOutput.format(z - cycle.depth)));
      if (!singleLine) {
        writeBlock(_TSA, _PRNUM, _VMS, _NMSP, _FA);
        writeBlock(_MVAR, _SETVAL, _MA, (conditional(cycleType == "probing-x-channel-with-island", _ID /*+ " " + _SZA*/)), _KNUM);
        writeBlock("CYCLE977");
      } else {
        writeBlock(
          "CYCLE977(" + ([_MVAR, _KNUM, "", _PRNUM, _SETVAL, "", "", _FA, _TSA,
            "", _ID, "", "", _MA, _NMSP, (properties.toolAsName ? tool.description : ""),
            "", "", "", "", "", "", "", "", "", _DMODE, ""].join(cycleSeparator))
        );
      }
      break;
    ...
    case "probing-y-channel-with-island":
      ...
      writeBlock(conditional(cycleType == "probing-y-channel", gMotionModal.format(1) + " " + zOutput.format(z - cycle.depth)));
      if (!singleLine) {
        writeBlock(_TSA, _PRNUM, _VMS, _NMSP, _FA);
        writeBlock(_MVAR, _SETVAL, _MA, (conditional(cycleType == "probing-y-channel-with-island", _ID /*+ " " + _SZA*/)), _KNUM);
        writeBlock("CYCLE977");
      } else {
        writeBlock(
          "CYCLE977(" + ([_MVAR, _KNUM, "", _PRNUM, _SETVAL, "", "",
            _FA, _TSA, "", (conditional(cycleType == "probing-y-channel-with-island", _ID)), _SZA, _SZO, _MA, _NMSP, (properties.toolAsName ? tool.description : ""),
            "", "", "", "", "", "", "", "", "", _DMODE, ""].join(cycleSeparator))
        );
      }
      break;
...
*/
    ...
    case "probing-y-wall":
      ...
      if (!singleLine) {
        ...
        writeBlock(
          "CYCLE977(" + ([_MVAR, _KNUM, "", _PRNUM, _SETVAL, "", "",
            _FA, _TSA, "", _ID, _SZA, _SZO, _MA, _NMSP, (properties.toolAsName ? tool.description : ""),
            "", "", "", "", "", "", "", "", "", _DMODE, ""].join(cycleSeparator))
        );
      }
      break;
    ...
    case "probing-xy-circular-hole-with-island":
      ...
      if (!singleLine) {
        writeBlock(_TSA, _PRNUM, _VMS, _NMSP, _FA);
        writeBlock(_MVAR, _SETVAL, (conditional(cycleType == "probing-xy-circular-hole-with-island", _ID /*+ " " + _SZA*/)), _KNUM);
        writeBlock("CYCLE977");
      } else {
        writeBlock(
          "CYCLE977(" + ([_MVAR, _KNUM, "", _PRNUM, _SETVAL, "", "",
            _FA, _TSA, "", (conditional(cycleType == "probing-xy-circular-hole-with-island", _ID)), _SZA, _SZO, _MA, _NMSP, (properties.toolAsName ? tool.description : ""),
            "", "", "", "", "", "", "", "", "", _DMODE, ""].join(cycleSeparator))
        );
      }
      break;
    case "probing-xy-circular-boss":
      ...
      if (!singleLine) {
        writeBlock(_TSA, _PRNUM, _VMS, _NMSP, _FA);
        writeBlock(_MVAR, _SETVAL, _ID, _KNUM);
        writeBlock("CYCLE977");
      } else {
        writeBlock(
          "CYCLE977(" + ([_MVAR, _KNUM, "", _PRNUM, _SETVAL, "", "",
    ...
    case "probing-xy-rectangular-boss":
      ...
      writeBlock(conditional(cycleType == "probing-xy-rectangular-hole", gMotionModal.format(1) + " " + zOutput.format(z - cycle.depth)));
      if (!singleLine) {
        writeBlock(_TSA, _PRNUM, _VMS, _NMSP, _FA);
        writeBlock(_MVAR, _SETV0, _SETV1, (conditional(cycleType == "probing-xy-rectangular-boss", _ID)), _KNUM);
        writeBlock("CYCLE977");
      } else {
        writeBlock(
          "CYCLE977(" + ([_MVAR, _KNUM, "", _PRNUM, "", _SETV0, _SETV1,
            _FA, _TSA, "", (conditional(cycleType == "probing-xy-rectangular-boss", _ID)),
            _SZA, _SZO, _MA, _NMSP, (properties.toolAsName ? tool.description : ""),
            "", "", "", "", "", "", "", "", "", _DMODE, ""].join(cycleSeparator))
        );
      }
    ...
    case "probing-xy-rectangular-hole-with-island":
      ...
      if (!singleLine) {
        writeBlock(_TSA, _PRNUM, _VMS, _NMSP, _FA);
        writeBlock(_MVAR, _SETV0, _SETV1, /*_SZA, _SZO,*/ _ID, _KNUM); // in the simulation you dont need _SZA and _SZO
        writeBlock("CYCLE977");
      } else {
        writeBlock(
          "CYCLE977(" + ([_MVAR, _KNUM, "", _PRNUM, "", _SETV0, _SETV1,
    ...
    case "probing-y-plane-angle":
      _MVAR += xyzFormat.format(105);
      _TSA = (!singleLine ? "_TSA=" : "") + xyzFormat.format(0.1); // angle tolerance (in the simulation he move to the second point with this angle)
      _RA = (!singleLine ? "_RA=" : "") + xyzFormat.format(0); // correction of angle, 0 dont rotate the table
      _MA += xyzFormat.format(cycleType == "probing-x-plane-angle" ? 201 : 102);
      _ID = (!singleLine ? "_ID=" : "") + cycle.probeSpacing; // distance between point
      _STA1 = (!singleLine ? "_STA1=" : "") + xyzFormat.format(0); // angle of the plane
      var _SETVAL = (!singleLine ? "_SETVAL=" : "") + xyzFormat.format((cycleType == "probing-x-plane-angle" ? x : y) + approach(cycle.approach1) * (cycle.probeClearance + tool.diameter / 2));
      ...
      if (!singleLine) {
        writeBlock(_TSA, _PRNUM, _VMS, _NMSP, _FA);
        writeBlock(_MVAR, _SETVAL, _MA, _ID, _RA, _STA1, _KNUM);
        writeBlock("CYCLE998");
      } else {
        writeBlock(
          "CYCLE998(" + ([_MVAR, _KNUM, _RA, _PRNUM, _SETVAL, _STA1,
            "", _FA, _TSA, _MA, "", _ID, "", "", "", "", _NMSP, "", _DMODE, ""].join(cycleSeparator))
        );
      }
      break;
    default:
