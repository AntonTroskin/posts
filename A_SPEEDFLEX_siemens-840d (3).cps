/**
  ...
  for DMF 260-7 //anton 07/28

  $Revision: 42788 a05bea3c119f90804593c2e45e1eb24adb4919db $
  $Date: 2020-05-14 05:33:37 $
...
properties = {
  ...
  autoBazes: true,
  xOffset:"-1000.0", // enter X-offset value for output in G10 block 
  yOffset:"-600.0", // enter Y-offset value for output in G10 block 
  zOffset:"-400.0", // enter Z-offset value for output in G10 block 
  paverttimasKampais: false, // ABC output 
  ...
  cycle800Mode: "192", // specifies the mode to use for CYCLE800
  cycle800SwivelDataRecord: "DMG", // specifies the label to use for the Swivel Data Record for CYCLE800
  useExtendedCycles: true, // use extended cycles //anton ??
...
propertyDefinitions = {
  ...
  paverttimasKampais:{group:0, title:"paversti kampais BC arba vektoriais A3 B3 C3", description:"Value for S", type:"integer"},
...
function getCycle800Config() {//anton added 192 pagal 57
  var options = [];
  ...
  case "57":
    ...
    break;
  case "192"://anton
    options.push(192, ["B", "C", "A"], EULER_YZX_R);//anton
    break;//anton
  case "45":
...
function onOpen() {
  ...
  if (properties.paverttimasKampais) { // pavertimas kampais
    // var aAxis = createAxis({coordinate:0, table:true, axis:[1, 0, 0], range:[-120.0001, 120.0001], preference:1});
    var bAxis = createAxis({coordinate:1, table:false, axis:[0, 1, 0], range:[-95.0001, 95.0001], preference:1});
  ...
  // NOTE: setup your home positions here // anton dmf home pos
  machineConfiguration.setRetractPlane(-13.1); // home position Z
  machineConfiguration.setHomePositionX(-600); // home position X
  ...
  if (true) {
    ...
    writeComment("SPEEDFLEX:   " + d.toLocaleDateString() + " " +
  ...
  if (properties.writeTools) {
    ...
    if (tools.getNumberOfTools() > 0) {
      ...
      for (var i = 0; i < getNumberOfSections(); ++i) {        
        ...
        comment += "T" + (properties.toolAsName ? "="  + "\"" + (tool.description.toUpperCase().replace(" ", "_")) + "\"" : toolFormat.format(tool.number));// + " " +
        ...
        if (zRanges[tool.number]) {
            comment += "    Zmin=" + zRanges[tool.number].getMinimum().toFixed(1);
            // var toolmzin = "Zmin=" + zRanges[tool.number].getMinimum().toFixed(1);
        }  
        // comment +=  "   Zmin=" + xyzFormat.format(zRanges.getGlobalZRange().getMinimum());
        ...
        if (tool.threadPitch) {
          comment += "  TAP x" + xyzFormat.format(tool.threadPitch); 
        }
  ...
  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(95), gFormat.format(64), gPlaneModal.format(17));
...
function getFeed(f) { //anton g95 here
  if (activeMovements) {
    var feedContext = activeMovements[movement];
    if (feedContext != undefined) {
      if (!feedFormat.areDifferent(feedContext.feed, f/spindleSpeed)) { //parametricfeed anton
  ...
  return feedOutput.format(f/spindleSpeed); // use feed value // f / spindleSpeed anton
}
...
function setWorkPlane(abc, turn) {
  ...
  if (useMultiAxisFeatures) {
    ...
    var TC = properties.cycle800SwivelDataRecord; //orig
    // var TC = "DMG";
    // var ST = 0; //orig
    var ST = 100000;
    var MODE = cycle800Config[0]; //orig    
    // var MODE = 192;
    var X0 = 0;
    var Y0 = 0;
    var Z0 = 0;
    ...
    // var DMODE = 0; // keep the previous plane active orig
    var DMODE = 100; // keep the previous plane active
    var papildomasKoef = 1; //anton
    ...
    if (!properties.useExtendedCycles) {
      ...
      writeBlock(
        ...
        [FR,
          ...
          xyzFormat.format(Z1),
          DIR,
          // DMODE].join(",") + ")" //orig anton
          DMODE,
          papildomasKoef].join(",") + ")" //anton
      );
    }
  } else {
    gMotionModal.reset();
  ...
  if (turn) {
    ...
    onCommand(COMMAND_LOCK_MULTI_AXIS); //anton testit
    //}
  }
}
...
function setAxisMode(_format, _output, _prefix, _value, _incr) { //anton testit
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    writeRetract(Z);
    writeRetract(Y); //anton safe Y
    // writeBlock("G0 B0 C=DC(0)"); //anton safe start pos
    writeBlock("TRAFOOF")

    if (newWorkPlane && useMultiAxisFeatures) {
      // setWorkPlane(new Vector(0, 0, 0), false); // reset working plane
      writeBlock("CYCLE800()") // reset working plane
    }
  }
  writeln("");
  ...
  if (insertToolCall) {
    ...
    if (showToolZMin) {
      if (is3D()) {
        ...
        irankioMinZ = ";   D=" + tool.diameter.toFixed(1) + "  Zmin=" + zRange.getMinimum().toFixed(3) + "  - " + tool.description; //anton prie irankio
      }
    }
    writeWords("VERKTYG(" + (properties.toolAsName ? "="  + "\"" + (tool.description.toUpperCase().replace(" ", "_")) + "\"" : toolFormat.format(tool.number) + ")"), dFormat.format(lengthOffset), irankioMinZ); 
    writeBlock(mFormat.format(6));
    if (tool.comment) {
      writeBlock(";", tool.comment);
    }
    ...
    //   if (nextTool) {
    //     writeBlock("T" + (properties.toolAsName ? "="  + "\"" + (nextTool.description.toUpperCase().replace(" ", "_")) + "\"" : toolFormat.format(nextTool.number)));
    //   } else {
    //     // preload first tool
    ...
    //         writeBlock("T=" + "\"" + (firstToolDescription.toUpperCase().replace(" ", "_")) + "\"");
    //       }
    //     } else {
    ...
    if (properties.preloadTool) {
      var nextTool = (properties.toolAsName ? getNextToolDescription(tool.description) : getNextTool(tool.number));
      if (nextTool) {
        writeBlock("T" + (properties.toolAsName ? "="  + "\"" + (nextTool.description.toUpperCase().replace(" ", "_")) + "\"    ; irankio paruosimas" : toolFormat.format(nextTool.number)));
      } else {
        // preload first tool
        var section = getSection(0);
        var firstToolNumber = section.getTool().number;
        ...
        if (properties.toolAsName) {
          if (tool.description != firstToolDescription) {
            writeBlock("T=" + "\"" + (firstToolDescription.toUpperCase().replace(" ", "_")) + "\"    ; irankio paruosimas");
          }
        } else {
          if (tool.number != firstToolNumber) {
            writeBlock("T" + toolFormat.format(firstToolNumber)); //
  ...
  if (properties.useCycle832) { //anton _finish _rough _semifinish :TODO
 ...
 //anton maxSpindleSpeed
   if ((insertToolCall ||
    ...
    if (spindleSpeed > properties.maxSpindleSpeed) {
      newSpindleSpeed = properties.maxSpindleSpeed; //anton
      writeWords("   ; S" + tool.spindleRPM + " - pagal iranki"); //sukiai pagal iranki Anton
      // warning(localize("Spindle speed exceeds maximum value."));
    }
    writeBlock(
      // sOutput.format(spindleSpeed), mFormat.format(tool.clockwise ? 3 : 4) // orig
  ...
  if (currentSection.isMultiAxis()) {
    ...
    if (insertToolCall) {
      ...
      if (!machineConfiguration.isHeadConfiguration()) {
        writeBlock(
          gAbsIncModal.format(90), // anton ?? gAbsIncModal.format(95),
...
// function onSpindleSpeed(spindleSpeed) { // ramp spindlespeed anton
...
function onCycle() {
  ...
  // writeBlock(gPlaneModal.format(17)); // anton 5xis ?? gal ijungt?
  ...
  if ((cycleType != "tapping") &&
      ...
      (cycleType != "inspect") &&
      !isProbeOperation()) {
    writeBlock(feedOutput.format(cycle.feedrate/spindleSpeed)); //for g95 anton
  }
  ...
  case "counter-boring":
    ...
    break;
  case "chip-breaking":
    // if (cycle.accumulatedDepth < cycle.depth) { //anton ?? chipbreak issiaiskinti
    if (false) {
      expandCurrentCycle = true;
    } else {
      if (RTP > getCurrentPosition().z) {
  ...
  case "reaming":
    ...
    var FFR = cycle.feedrate/spindleSpeed; //anton g95
    var RFF = cycle.retractFeedrate/spindleSpeed; //anton g95
...
function onSectionEnd() {
  ...
  if (!isLastSection() && (getNextSection().getTool().coolant != tool.coolant)) {
    setCoolant(COOLANT_OFF);
    onCommand(COMMAND_STOP_SPINDLE); //anton
  }
  writeBlock(gPlaneModal.format(17));
...
function onClose() {
  ...
  writeln("")
  writeComment("Programos pabaiga") //anton
  onCommand(COMMAND_OPTIONAL_STOP);
