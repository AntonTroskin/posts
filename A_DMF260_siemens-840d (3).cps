...
function onOpen() {
  ...
  if (true) {
    ...
    writeComment("DMF 260-7:   " + d.toLocaleDateString() + " " +
  ...
  if (insertToolCall) {
    ...
    if (showToolZMin) {
      if (is3D()) {
        ...
        irankioMinZ = ";    D=" + tool.diameter.toFixed(1) + "   Zmin=" + zRange.getMinimum().toFixed(3); //anton prie irankio
      }
    }
    writeWords("T" + (properties.toolAsName ? "="  + "\"" + (tool.description.toUpperCase().replace(" ", "_")) + "\"" : toolFormat.format(tool.number)), dFormat.format(lengthOffset), irankioMinZ); 
